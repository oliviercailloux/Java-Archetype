...
* `mvn archetype:generate -DarchetypeGroupId=io.github.oliviercailloux -DarchetypeArtifactId=java-se-1.8-archetype -DgroupId=mygroupid -DartifactId=myartifactid -DinteractiveMode=false` fully automates the creation of a new project by also indicating the desired group and artifact ids.
* ``uuid=\`cat /proc/sys/kernel/random/uuid` ; mvn archetype:generate -DarchetypeGroupId=io.github.oliviercailloux -DarchetypeArtifactId=java-se-1.8-archetype -DgroupId=mygroupid -DartifactId=artifact_${uuid:0:3} -DinteractiveMode=false`` in Bash, creates a project with a partly random artifactId.
...
(Same instructions apply to Eclipse Neon and Oxygen.)
